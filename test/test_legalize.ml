...
let create_i64_add_function () =
  let open Mirl in
  Function.build ~name:"test_i64_add" ~params:["a", Type.I64; "b", Type.I64] 
    (fun builder params ->
      let a, b = match params with
        | [(_, a, _); (_, b, _)] -> a, b
...
let create_i64_sub_function () =
  let open Mirl in
  Function.build ~name:"test_i64_sub" ~params:["a", Type.I64; "b", Type.I64] 
    (fun builder params ->
      let a, b = match params with
        | [(_, a, _); (_, b, _)] -> a, b
        | _ -> failwith "Expected 2 parameters"
...
let create_i64_immediate_function () =
  let open Mirl in
  Function.build ~name:"test_i64_immediate" ~params:[] 
    (fun builder _params ->
      Function.Builder.add_block' builder (fun block_builder ->
        let result = Function.Builder.fresh_register builder ~ty:Type.I64 in
        let instructions = [
          Instruction.Set { dst = result; value = 0x12345678 }; (* Large 32-bit value *)
          Instruction.Return result
        ] in
        Block.Builder.push_many block_builder instructions)
      [@nontail])
...
let create_i64_move_function () =
  let open Mirl in
  Function.build ~name:"test_i64_move" ~params:["x", Type.I64] 
    (fun builder params ->
      let x = match params with
        | [(_, x, _)] -> x
      ...
      Function.Builder.add_block' builder (fun block_builder ->
        let result = Function.Builder.fresh_register builder ~ty:Type.I64 in
        let instructions = [
          Instruction.Mov { dst = result; src = x };
          Instruction.Return result
        ] in
        Block.Builder.push_many block_builder instructions)
      [@nontail])
...
let create_mixed_i32_i64_function () =
  let open Mirl in
  Function.build ~name:"test_mixed" ~params:["a", Type.I32; "b", Type.I64] 
    (fun builder params ->
      let a, b = match params with
        | [(_, a, _); (_, b, _)] -> a, b
        | _ -> failwith "Expected 2 parameters"
      ...
      Function.Builder.add_block' builder (fun block_builder ->
        ...
        let instructions = [
          ...
          Instruction.Add { dst = result; src1 = b; src2 = temp }; (* This will trigger i64+i32 handling *)
          Instruction.Return result
        ] in
        Block.Builder.push_many block_builder instructions)
      [@nontail])
