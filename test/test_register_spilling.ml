...
let create_guaranteed_spill_function () =
  let open Mirl in
  Function.build ~name:"guaranteed_spill" ~params:[] (fun builder _params ->
...
let create_extreme_i64_spill_function () =
  let open Mirl in
  Function.build ~name:"extreme_i64_spill" ~params:[] (fun builder _params ->
    Function.Builder.add_block' builder (fun block_builder ->
      (* Create 8 i64 registers - on ARM32 this becomes 16 i32 registers *)
      let i64_regs =
        Array.init 8 ~f:(fun _ -> Function.Builder.fresh_register builder ~ty:Type.I64)
      in
      let init_instructions =
        Array.mapi i64_regs ~f:(fun i reg ->
...
let create_call_spill_function () =
  let open Mirl in
  Function.build ~name:"call_spill" ~params:[] (fun builder _params ->
    Function.Builder.add_block' builder (fun block_builder ->
...
let analyze_spilling_requirements func_name create_func =
  ...
  let test_capacity capacity =
    ...
    let module TestReg = struct
      ...
      let callee_saved =
        Set.of_list (List.init (capacity / 2) ~f:(fun i -> i + (capacity / 2)))
      ;;
    end
    in
...
let%expect_test "current spilling limitation demonstration" =
  ...
  printf
    "The register allocator currently fails with 'todo: spill' when register pressure \
     exceeds capacity.\n";
  printf "This affects:\n";
