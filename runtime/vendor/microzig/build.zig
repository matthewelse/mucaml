...
pub fn MicroBuild(port_select: PortSelect) type {
    return struct {
        ...
        pub fn add_library(mb: *Self, options: CreateFirmwareOptions) *StaticLibrary {
            const b = mb.dep.builder;
            ...
            const fw = mb.builder.allocator.create(StaticLibrary) catch @panic("out of memory");
            fw.* = .{
                .mb = mb,
                .core_mod = core_mod,
                .artifact = mb.builder.addLibrary(.{
                    .name = options.name,
                    .root_module = b.createModule(.{
                        .optimize = options.optimize,
                        .target = zig_resolved_target,
                ...
                .app_mod = app_mod,
                .target = target,
                .emitted_files = StaticLibrary.EmittedFiles.init(mb.builder.allocator),
            };
        ...
        pub const StaticLibrary = struct {
            ...
            pub fn add_app_import(fw: *StaticLibrary, name: []const u8, module: *Build.Module, options: AppDependencyOptions) void {
            ...
            pub fn add_include_path(fw: *StaticLibrary, path: LazyPath) void {
                fw.artifact.addIncludePath(path);
            }

            /// Adds a system include path to the firmware.
            pub fn add_system_include_path(fw: *StaticLibrary, path: LazyPath) void {
                fw.artifact.addSystemIncludePath(path);
            }

            /// Adds a c source file to the firmware.
            pub fn add_c_source_file(fw: *StaticLibrary, source: Build.Module.CSourceFile) void {
                fw.artifact.addCSourceFile(source);
            }

            /// Adds options to your application.
            pub fn add_options(fw: *StaticLibrary, module_name: []const u8, options: *Build.Step.Options) void {
                fw.app_mod.addOptions(module_name, options);
            }

            /// Adds an object file to the firmware.
            pub fn add_object_file(fw: *StaticLibrary, source: LazyPath) void {
